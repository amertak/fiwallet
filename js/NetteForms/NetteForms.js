 ...
 *
 * @author Radek Ježdík, MartyIX, David Grudl, pavelplzak, Robyer, Pavel Kouřil
 *
 * @changelog
 *   Pavel Kouřil, 29.3.2014:
 *     - fixed bug with tags with attribute name being the same as form inputs
 *   Pavel Kouřil, 29.3.2014:
 *     - fixed error adding for checkbox lists and radio lists
 *   Robyer, 8.8.2014:
...
LiveForm.getErrorElementId = function (el) {
    if (!el.id) {
        return "frm-" + Nette.webalize(el.name) + this.options.messageIdPostfix;
    } else {
        return id = el.id + this.options.messageIdPostfix;;
    }
};
...
LiveForm.removeError = function(el) {
    ...
    var err_el = document.getElementById(this.getErrorElementId(el));
...
LiveForm.getMessageElement = function(el) {
    var id = this.getErrorElementId(el);
    ...
    if (!error) {
        ...
        if(!this.hasClass(el, this.options.showErrorApartClass)) {
            ...
            if (ancestor = findAncestor(el, this.options.showMessageClassOnParent)) {
                ancestor.appendChild(error);
            }
...
Nette.validateControl = function(elem, rules, onlyCheck) {
    if (elem.form && elem.form.id) {
        var formId = elem.form.id;
        if (elem.name && elem.tagName == "INPUT") { // RadioNodeList
            elem = document.querySelectorAll("form#" + formId + " input[name='" + elem.name + "']")[0];
        }
        if (elem.name && elem.tagName == "SELECT") {
            elem = document.querySelectorAll("form#" + formId + " select[name='" + elem.name + "']")[0];
        }
        if (elem.name && elem.tagName == "TEXTAREA") {
            elem = document.querySelectorAll("form#" + formId + " textarea[name='" + elem.name + "']")[0];
        }
    }
